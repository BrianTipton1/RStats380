...
filter(gapminder, lifeExp > 60, country %in% c("United Kingdom", "Vietnam"), year > 1990)
...
new_data <- gapminder %>%
  mutate(gapminder,gdpPercap_lifeExp = gdpPercap / lifeExp)

select(gapminder, gdpPercap_lifeExp)

summary_data <- new_data %>%
  summarise(mean_gdpPercap_lifeExp = mean(gdpPercap_lifeExp))

grouped_summary <- gapminder %>%
  group_by(continent) %>%
...
filtered_mlb16 <- mlb16.data %>%
  filter(STL == "Cardinals", H > 1400, Lg == "NL")

sorted_mlb_data <- mlb_data %>% arrange(desc(HR))

sorted_by_rbi <- mlb_data %>% arrange(desc(RBI))

average_rbi_by_league <- mlb_data %>%
  group_by(Lg) %>%
  summarise(avg_RBI = mean(RBI))
